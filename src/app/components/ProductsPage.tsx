...
const CATEGORY_MAP = [
  ...
  { id: "bumpers", label: "BUMPERS & BODY KITS", desc: "Aggressive designs, durable plastics, and track-inspired aero." },
  { id: "mirrors", label: "MIRROR COVERS", desc: "Carbon fiber overlays and glossy replacement caps." },
  { id: "lighting", label: "AMBIENT LIGHTING", desc: "Vibrant RGB underglows and multi-zone interior lighting." },
  { id: "spoilers", label: "SPOILERS & WINGS", desc: "High-density carbon wings, aggressive decklid spoilers." },
  { id: "wheels", label: "RIMS & WHEELS", desc: "Lightweight sport alloys and staggered offset rims." },
  { id: "interior", label: "CARBON INTERIOR", desc: "Hand-crafted carbon steering wheels and interior trims." }
];

export function ProductsPage({ initialCategory = "all", initialQuery = "", onAddToCart, onBackToHome }: ProductsPageProps) {
  ...
  const activeCategoryInfo = useMemo(() => {
    return CATEGORY_MAP.find(c => c.id === selectedCategory) || CATEGORY_MAP[0];
  }, [selectedCategory]);
  ...
  const processedProducts = useMemo(() => {
    ...
    return result;
  }, [selectedCategory, compatibility, searchQuery, priceRange, sortOption]);
    ...
    <div style={{ background: "#080808", minHeight: "100vh", paddingTop: "80px" }}>
      ...
      >
        ...
        <div className="flex items-center justify-center gap-2 mb-4 text-xs font-semibold tracking-wider text-neutral-500 uppercase">
          <span className="hover:text-white cursor-pointer transition-colors" onClick={onBackToHome}>Home</span>
                ...
                <div className="flex flex-col gap-2 text-xs" style={{ color: "#888" }}>
                  {["ALL", "Civic", "Corolla", "Yaris", "BRV"].map((model) => (
                    <label key={model} className="flex items-center gap-2 cursor-pointer hover:text-white transition-colors">
                      ...
                      />
                      <span>{model === "ALL" ? "All Cars" : `Honda ${model}` && model === "Corolla" ? "Toyota Corolla" : model === "Yaris" ? "Toyota Yaris" : `Honda ${model}`}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-6 pb-4 border-b border-neutral-900">
                    ...
                    <button onClick={() => setShowFiltersMobile(false)} className="text-neutral-500 font-bold text-sm">CLOSE</button>
                  </div>
                ...
                <button
                  ...
                  className="w-full py-3 rounded font-bold text-xs text-white tracking-wider"
            ...
            {processedProducts.length > 0 ? (
              ...
              >
                ...
                <button
                  ...
                  style={{ background: "#e8192c", cursor: "pointer" }}
                >
                  RESET ALL FILTERS
                </button>
              </div>
